...
def detect_expression(path,name,fps):
    ...
    for vio in fileList:
            ...
            for i,interval in enumerate(pp):
                data.loc[i+j,'vid'] = vio #去掉 _aligned
            ...
            j=i+j+1
    data.to_csv(name+'_pred_gradient03'+".csv",index=False)

def main():
    # path_samm ='E:/MEGC2023 test/openface_sammalign'  #'/space/qwf/sammtest/SAMM_data/'
    # detect_expression(path_samm,'samm',200)
    path_cas ='/home/data/CZP/2024megc_test/CAS_align' #'E:/openfaceprocess_22test/cas3'#'D:/graduate/medata/CAS_Test_cropped' #  #'/space/qwf/cas3test/CAS_data/'
