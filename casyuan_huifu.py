    ...
    for i in range(len(flow1_total_fenxi)):
        ...
        if (st_arglow < start):
            for j in range(start - 1, -1, -1):
                ...
                #if (flow1_total_edm[j] - st_low < 0.38 * (high - st_low)):
                    start = j
                    break
...
def adjust_intervals(intervals, data, gradient_threshold=0.3):
    adjusted_intervals = []
...
def process(flow1_total, yuzhi1, yuzhi2, position, xuhao, k, a, totalflow):
    ...
    #flow1_total 光流 flow1_total_edm1 过滤 flow1_total_edm2 进一步平滑 imf_sum1 减去bias
    flow1_total_fenxi = flow1_total_fenxi + (k - hh) + a

    flow1_total_fenxi = adjust_intervals(flow1_total_fenxi, flow1_total_edm1, gradient_threshold=0.3)
...
#     c2 = -0.1
#     yuzhi2 = yuzhi2 + c2
#     flow1_total = draw_line(flow1_total)
#     flow1_total = np.array(flow1_total)
#     position = position + str(xuhao) + "----"
...
#     # 调整区间
#     flow1_total_fenxi = adjust_intervals(flow1_total_fenxi, flow1_total, gradient_threshold=0.1)
...
def draw_roiline19(path1, path2, qian, hou, fs):  # 与16相比再增加两个位置眼睑部位
    ...
    while (k < len(fileList)):
        ...
        for i in fileList:
            ...
            if (k == end):
                ...
                totalflowmac = []
                a = 1
                totalflow=process(flow1_total,0.7,1.8,"left_eye",0,k,a,totalflow)
                totalflow = process(flow1_total1, 0.7, 1.8, "left_eye", 1, k, a, totalflow)
                totalflow = process(flow1_total2, 0.7, 1.8, "left_eye", 2, k, a, totalflow)
                totalflow = process(flow1_total3, 0.7, 1.8, "left_eye", 3, k, a, totalflow)

                totalflow=process(flow3_total,0.7,1.8,"right_eye",0,k,a,totalflow)
                totalflow = process(flow3_total1, 0.7, 1.8, "right_eye", 1, k, a, totalflow)
                totalflow = process(flow3_total2, 0.7, 1.8, "right_eye", 2, k, a, totalflow)
                totalflow = process(flow3_total3, 0.7, 1.8, "right_eye", 3, k, a, totalflow)

                totalflow=process(flow4_total,0.7,1.85,"mouth",0,k,a,totalflow)
                totalflow = process(flow4_total1, 0.7, 1.85, "mouth", 1, k, a, totalflow)
                totalflow = process(flow4_total2, 0.7, 1.85, "mouth", 2, k, a, totalflow)
                totalflow = process(flow4_total3, 0.7, 1.85, "mouth", 3, k, a, totalflow)
                totalflow = process(flow4_total4, 0.7, 1.85, "mouth", 4, k, a, totalflow)
                totalflow = process(flow4_total5, 0.7, 1.85, "mouth", 5, k, a, totalflow)

                totalflow = process(flow5_total1, 0.7, 2.1, "nose", 1, k, a, totalflow)
                totalflow = process(flow5_total2, 0.7, 2.1, "nose", 2, k, a, totalflow)

                # totalflow = np.array(nms2(totalflow, 0.29))  # 把所有通道融合起来
                # totalflow = np.array(nms2(totalflow, 0.29))
                totalflow = np.array(nms2(totalflow, 0.34))  # 把所有通道融合起来
                totalflow = np.array(nms2(totalflow, 0.34))
    ...
    #label_video_update = np.array(nms2(label_video_update, 0.29))#0.2
   #试一下一个
    #label_video_update = np.array(nms2(label_video_update, 0.29))

    label_video_update = np.array(nms2(label_video_update, 0.34))  # 0.2
    # label_video_update = np.array(nms2(label_video_update, 0.2))
    start = 0
    end = 0
    #排除掉s,e都是0的  以及段之间距离太近的
